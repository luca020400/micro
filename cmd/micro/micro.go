...
func LoadInput(args []string) []*buffer.Buffer {
	...
	if len(files) > 0 {
		...
		for i := 0; i < len(files); i++ {
			buf, err := buffer.NewBufferFromFileAtLocWithSearchText(files[i], btype, command)
			if err != nil {
				screen.TermMessage(err)
				continue
			}
		...
		buffers = append(buffers, buffer.NewBufferFromStringAtLocWithSearchText(string(input), filename, btype, command))
	} else {
